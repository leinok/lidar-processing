...
def readLidarBin(file_path = '/mnt/data_0/kitti/training/velodyne/', kitti_format = True, idx = 0):
    if os.path.isdir(file_path):
	if kitti_format is True:
	    bin_name = os.path.join(bin_folder, '%06.bin'%idx)
    elif os.path.isfile(file_path):
	bin_name = file_path
    else:
	print "{} is not dir nor regular file!".format(file_path)
	return None
    bin_data = np.fromfile(bin_name, dtype = np.float32).reshape(-1, 4)
    #bin_data = bin_data / 100.		# The data from out car is cm units based.
    return bin_data
...
# Stay with the setting of the paper.
# Features in (M + 2) channels, namely, M height maps, density and intensity
...
def generateBvMap(bin_data, M = 1, boundary = [1.0, 21.0, -20, 20, -2.0, 0.4], resolution = [0.1, 0.1]):
    ...
    bv_map = np.zeros((img_width, img_height, M + 2))
    # Height Normalization
    bin_data[:, 2] = (bin_data[:, 2] - boundary[4]) / (boundary[5] - boundary[4])
    norm_slice_height = 1.0 / M
 
    max_height_map = np.zeros((img_width, img_height))
	...
	# Intensity, in the range of (0, 1) 
	if z > max_height_map[x_bv][y_bv]:
	    max_height_map[x_bv][y_bv] = z
	    bv_map[x_bv][y_bv][M+1] = r	
...
#    boundary = [1.0, 21.0, -20, 20, -2.0, 0.4]
    boundary = [0.0, 60.0, -30, 30, -2.0, 0.4]
    feat_format = "npy"
    bin_data = readLidarBin(bin_folder, True, idx)
    if bin_data is None:
	return
...
#    bin_folder = '/mnt/data_0/SAIC_DATA/VLP32_2017_1103/'
    save_directory = os.path.join("/mnt/data_0/kitti/training/", 'test_new_bv_feat{}'.format(M + 2))
